...
func (s *NodeKeySignature) verifySignature(nodePublic key.NodePublic, verificationKey Key) error {
	switch s.SigKind {
	case SigDirect:
		var stored key.NodePublic
		if err := stored.UnmarshalBinary(s.Pubkey); err != nil {
			return fmt.Errorf("unmarshalling public: %v", err)
		}
		if nodePublic != stored {
			return errors.New("signature references a different node key")
		}

	default:
		return fmt.Errorf("unhandled signature kind: %v", s.SigKind)
	}
